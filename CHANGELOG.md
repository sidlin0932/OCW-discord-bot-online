...
### 新增功能 (New Features)
- **每日趨勢圖自動發送**:
    - 每天半夜 12:00 (UTC+8) 自動將 `/trends` 頁面連結發送至 Bot 專區。
    - 讓大家能持續關注學習進度與全班動態。
- **手動發送指令 (`/sendchart`)**:
    - 新增指令讓用戶可隨時手動分享趨勢圖。
### 改進項目 (Improvements)
- **時間格式優化**: CHANGELOG 時間戳移除秒數，改為 `HH:MM` 格式，更簡潔易讀。

## [1.3.0] - 2025-11-27 03:30
### 安全性更新 (Security)
